...
function collectIntersectingTriangles(a, b) {
  ...
  _matrix$2.copy(a.matrixWorld).invert().multiply(b.matrixWorld)
...
function pushBarycoordInterpolatedValues(
  v0,
  v1,
  v2,
  baryCoordTri,
...
function performSplitTriangleOperations(
  ...
  _matrix$1.copy(b.matrixWorld).invert().multiply(a.matrixWorld)
...
class Evaluator {
  ...
  evaluate(a, b, operation, targetBrush = new Brush()) {
    ...
    const aGroups =
      !useGroups || a.geometry.groups.length === 0
        ? [{ start: 0, count: Infinity, materialIndex: 0 }]
        : a.geometry.groups.map((group) => ({ ...group }))

    const bGroups =
      !useGroups || b.geometry.groups.length === 0
        ? [{ start: 0, count: Infinity, materialIndex: 0 }]
...
function generateRandomTriangleColors(geometry) {
  ...
  for (let i = 0, l = array.length; i < l; i += 9) {
    color.setHSL(Math.random(), MathUtils.lerp(0.5, 1.0, Math.random()), MathUtils.lerp(0.5, 0.75, Math.random()))
